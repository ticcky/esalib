 ...
 */
public class HeapSort<T, D extends Comparable<D>> {
	...
	private void buildMaxHeap(D[] A, T[] B) {
		heapSize = A.length;
		for (int i = (int) Math.floor(heapSize / 2.0D); i > 0; i--)
			maxHeapify(A, B, i);
	}
	...
	private void maxHeapify(D[] A, T[] B, int i) {
		l = 2 * i;
		r = 2 * i + 1;
